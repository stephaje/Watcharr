...
func plexGetLibraries(userThirdPartyAuth string) ([]string, error) {
	...
	ret := make([]string, len(resp.MediaContainer.Directory))

	for i, item := range resp.MediaContainer.Directory {
		ret[i] = item.Key
	}
...
func plexGetLibraryItems(userThirdPartyAuth string, library string) ([]PlexMetadata, error) {
	...
	if err != nil {
		slog.Error("plexGetLibraryItems: Plex Librarie Items API request failed", "error", err)
